...
void handleRoot() {
  String page = "<h1>Wi-Fi Jammer</h1><p>Available Networks:</p><ul>";
  int numNetworks = WiFi.scanNetworks();
  for (int i = 0; i < numNetworks; i++) {
  ...
  page += "<p><b>Note:</b> Works only on 2.4GHz Wi-Fi (not 5GHz, WPA3 might not be affected).<br>";
  page += "The attack is temporary (devices will try to reconnect).<br>";
  page += "Not effective if management frame protection (MFP) is enabled on the target Wi-Fi.</p>";
...
void setup() {
  ...
  server.begin();
  Serial.println("Web Control Panel Ready");
}
